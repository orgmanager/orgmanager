                ...
                <div class="join">
                  ...
                  @endif
                    <form id="join-form" method="POST" href="{{ url('join/'.$org->id) }}">
                      {{ csrf_field() }}
                      @if ($org->password != null && trim($org->password) != "")
                      <input type="password" name="org_password" id="org_password" class="textbox" placeholder="@lang('join.pplace')"><br><br>
                      @endif
                      <div id='recaptcha' class="g-recaptcha" data-sitekey="{{ config('recaptcha.key') }}" data-callback="submitForm" data-size="invisible"></div>
                      <button class="btn btn-social btn-github" type="submit" id="btnSubmit"><i class="octicon octicon-mark-github"></i> @lang('join.button')!</button>
                    </form>
                </div>
            </div>
        </div>
